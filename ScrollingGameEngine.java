...
public abstract class ScrollingGameEngine {
    ...
    protected ArrayList<Entity> checkDeflectCollision(Entity toCheck){
        ArrayList<Entity> deflectCollisions = new ArrayList<Entity>();
        for (Entity e : displayList){
            //the argument Entity won't be detected as colliding with
            //itself if it is currently in the displayList
            if (e != toCheck && toCheck.frontIsCollidingWith(e)){
                deflectCollisions.add(e);
            }
        }
        return deflectCollisions;            
    }    
